 ...
 */
const contextMenuCallbacks = new Map<string, (...args: any[]) => void>();
...
export function useElectron() {
  ...
  const menu = {
    show: (items?: Electron.MenuItemConstructorOptions[]) => {
      const ipcItems = items?.map((item) => {
        if (typeof item.click === "function") {
          const id = item.id || `menu-item-${Math.random().toString(36).substring(7)}`;
          contextMenuCallbacks.set(id, item.click as (...args: any[]) => void);
          return { ...item, id, click: undefined }; // Remove function, only send id
        }
        return item;
      });
      electron!.showContextMenu(ipcItems);
    },
  };
  ...
  onMounted(async () => {
    if (electron) {
      ...
      electron.onContextMenuClick((id) => {
        const cb = contextMenuCallbacks.get(id);
        if (cb)
          cb();
      });
    }
